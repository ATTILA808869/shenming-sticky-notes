      ...
      <section className="grid gap-10 lg:grid-cols-[1fr_0.9fr]">
        <div className="grid gap-4">
          <div className="relative aspect-square overflow-hidden rounded-lg border border-orange-100 bg-white shadow-soft">
            <Image src={images[0] ?? "https://placehold.co/900x900/FFE2CF/2E2A27/png?text=商品"} alt={product.name} fill priority className="object-cover" />
          </div>
        ...
        <section className="temple-card self-start p-6">
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => <span key={tag} className="rounded-sm border border-red-100 px-2 py-1 text-xs font-bold text-temple-red">{tag}</span>)}
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-neutral-600">
            <span className="flex items-center gap-1 text-temple-gold"><Star size={16} fill="currentColor" /> {product.rating.toFixed(1)}</span>
